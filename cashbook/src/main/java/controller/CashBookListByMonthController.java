...
public class CashBookListByMonthController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("[CashBookListByMonthController.doGet] year : " + y);
		System.out.println("[CashBookListByMonthController.doGet] month : " + m);
		...
		*/
		...
		firstDay.set(Calendar.MONTH, m - 1); // 자바 달력 API는 1월을 0으로, 2월을 1로, ... 12월을 11로 설정되어있다.
		firstDay.set(Calendar.DATE, 1); // ex) 2022.04.01
		int dayOfWeek = firstDay.get(Calendar.DAY_OF_WEEK);
		// dayOfWeek는 일 1, 월 2, ..., 토 7이다.
		// startBlank는 일 0, 월 1, ..., 토 6 이었으면 좋겠다.
		// 1) startBlank
		int startBlank = dayOfWeek - 1; // 일 0, 월 1, ..., 토 6 -> 1일의 요일을 이용하여 구한다
		// 2) endDay / 마지막 날짜는 자바 달력 API를 이용하여 구하자
		...
		// 2) 모델값(월별 가계부 리스트)을 반환하는 비즈니스 로직(모델) 호출
		CashBookDao cashbookDao = new CashBookDao();
		List<Map<String, Object>> list = cashbookDao.selectCashBookListByMonth(y, m);
		
		// 달력 출력에 필요한 모델값(1), 2), 3), 4)) + 데이터베이스에서 반환된 모델값(list, y출력년도, m출력월) + 오늘 날짜(today)
